...
use pxfm::{
    f_cathethus, f_cathetusf, f_cosm1, f_cosm1f, f_cotf, f_cotpi, f_cotpif, f_digamma, f_digammaf,
    f_erfcinv, f_erfcinvf, f_erfcx, f_erfcxf, f_erfinv, f_erfinvf, f_expf, f_expm1, f_i0, f_i0f,
    f_i1, f_i1f, f_i2, f_i2f, f_j0, f_j1, f_k0, f_k0f, f_k1, f_k1f, f_k2f, f_lgammaf, f_log1pf,
    f_log1pmx, f_log1pmxf, f_sin, f_sinmx, f_sinmxf, f_tanf, f_tanpi, f_tanpif, f_trigamma,
    f_trigammaf, f_y0, f_y0f, f_y1, f_y1f,
};
use rand::Rng;
...
fn test_f32_against_mpfr_multithreaded() {
    ...
    //
    //     let expected_sin_pi = cathethusf(x, y);
    ...
    let start_bits = (1e307f64).to_bits();
    ...
    (start_bits..=end_bits).into_par_iter().for_each(|bits| {
        ...
        let y = rand::rng().random_range(1e305f64..1e306f64);

        let expected = cathethus(x, y); //Float::with_val(90, (x as f64).trigamma());
        let actual = f_cathethus(x, y);
