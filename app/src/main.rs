...
use pxfm::{
    ...
    f_i0, f_j0, f_j0f, f_j1, f_j1f, f_k0, f_k1, f_log, f_rcbrtf, f_rerf, f_rerff, f_rsqrtf, f_secf,
    f_sincf, f_sincospif, f_sinf, f_sinpi, f_sinpif, f_tanf, f_tgammaf, f_y0, f_y0f, f_y1, f_y1f,
};
...
fn test_f32_against_mpfr_multithreaded() {
    ...
    let start_bits = (-12f32).to_bits();
    let end_bits = (-38f32).to_bits() - 1;
    ...
    (start_bits..=end_bits).into_par_iter().for_each(|bits| {
        ...
        let expected_sin_pi = Float::with_val(53, x).gamma(); //compute_besselk(x as f64).unwrap(); //Float::with_val(90, statrs::function::erf::erf_inv(x as f64));
        let actual = f_tgammaf(x);
    ...
    // let start_bits = (-0.001f64).to_bits();
    ...
    //     // };
    //
    //     let expected = compute_besselk(x).unwrap(); //Float::with_val(90, x).erf().recip();
