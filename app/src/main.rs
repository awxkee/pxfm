...
use pxfm::{
    f_cotf, f_cotpi, f_cotpif, f_digamma, f_digammaf, f_erfcinvf, f_erfinv, f_erfinvf, f_i0, f_i0f,
    f_i1, f_i1f, f_i2f, f_j0, f_j1, f_k0, f_k0f, f_k1, f_k1f, f_tanf, f_tanpi, f_tanpif, f_y0,
    f_y1,
};
...
fn test_f32_against_mpfr_multithreaded() {
    ...
    let start_bits = (-0.1f32).to_bits();
    let end_bits = (-7.75f32).to_bits();
    ...
    // let start_bits = (0.24f64).to_bits();
    ...
    //     // };
    //
    //     let expected = Float::with_val(90, x).digamma();
    //     let actual = f_digamma(x);
    //
